...
io.on("connection", (socket) => {
  ...
  socket.on("streamData", (data) => {
    try {
      console.log(
        `Received stream data chunk of size: ${data.byteLength} bytes`
      );

      if (isFirstChunk) {
        console.log(
          `First chunk data (first 100 bytes): ${Buffer.from(data).toString(
            "hex",
            0,
            100
          )}`
      ...
      socket.emit(
        "streamError",
        "An unexpected error occurred while processing the stream."
      );
    }
  });
  ...
  socket.on("disconnect", () => {
    ...
    isFFmpegRunning = false;
    console.log(
      `Stream ${streamKey} will expire in ${STREAM_EXPIRY_TIME / 1000} seconds`
    );
  });
});
...
app.use("/streams/:streamId", (req, res, next) => {
  ...
  fs.readFile(filePath, (err, data) => {
    ...
    console.log(
      `File signed. Signature (first 20 chars): ${signature.substr(0, 20)}...`
    );
